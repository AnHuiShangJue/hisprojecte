...
import com.ahsj.userinfor.common.SysConstants;
import com.ahsj.userinfor.common.utils.JsonUtils;
import com.ahsj.userinfor.controller.BaseLoginUser;
import com.ahsj.userinfor.dao.*;
import com.ahsj.userinfor.entity.*;
import com.ahsj.userinfor.entity.TranslateModel.OrganizationTranslate;
...
public class OrganizationServiceImpl extends BaseServiceImpl implements OrganizationService {
    ...
    private static final String AHSJ_ORGANIZATION = "123456";
...
//                sysOrganization.setType(companyInfo.getType());
            ...
            BeanUtils.copyProperties(sysOrganization, translate);
            translateModels.setUserId(loginUser.getId());
            translateModels.setOrganizationTranslate(translate);
            amqpTemplat.convertAndSend("com.ahsj.addOrganizationTranslate", JsonUtils.serialize(translateModels));
            log.info(JsonUtils.serialize(translateModels));
            log.info("--------------------组织模块新增翻译发送结束--------------------------");

            amqpTemplat.convertAndSend("com.ahsj.addOrganization", AHSJ_ORGANIZATION);
            ...
            translateModels.setUserId(loginUser.getId());
            translateModels.setOrganizationTranslate(translate);
            amqpTemplat.convertAndSend("com.ahsj.updateOrganizationTranslate", JsonUtils.serialize(translateModels));
            log.info(JsonUtils.serialize(translateModels));
            log.info("--------------------组织模块修改翻译发送结束--------------------------");
            amqpTemplat.convertAndSend("com.ahsj.updateOrganization", AHSJ_ORGANIZATION);
    ...
    public Message delete(String[] id) throws Exception {
        for (int i = 0; i < id.length; i++) {
            ...
            log.info("--------------------组织模块删除翻译发送开始--------------------------");
            TranslateDelete translateDelete = new TranslateDelete();
            translateDelete.setId(Long.parseLong(id[i]));
            translateDelete.setModel(Constants.TRANSLATE_SYS_ORGANIZATION);
            ...
            log.info("--------------------组织模块删除翻译发送结束--------------------------");
        }
        amqpTemplat.convertAndSend("com.ahsj.deleteOrganization", AHSJ_ORGANIZATION);
